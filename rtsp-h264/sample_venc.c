...
HI_S32 SAMPLE_VENC_VPSS_Init(VPSS_GRP VpssGrp, HI_BOOL* pabChnEnable, DYNAMIC_RANGE_E enDynamicRange,PIXEL_FORMAT_E enPixelFormat,SIZE_S stSize[],SAMPLE_SNS_TYPE_E enSnsType)
{
    ...
    for(i=0; i<VPSS_MAX_PHY_CHN_NUM; i++)
    {
        ...
        {
            stVpssChnAttr[i].u32Width                     = stSize[i].u32Height; //stSize[i].u32Width;
            stVpssChnAttr[i].u32Height                    = stSize[i].u32Width; //stSize[i].u32Height;
    ...
    s32Ret = HI_MPI_VPSS_SetChnRotation(0 , 0, ROTATION_270);
    if(s32Ret != HI_SUCCESS)
    {
        SAMPLE_PRT("rotate vpss fail for %#x!\n", s32Ret);
    }
...
void *SAMPLE_VENC_H265_H264(void *p)
{
    ...
    ******************************************/
    ...
    while(1) {
		usleep(10000);
		// SAMPLE_PRT("SAMPLE_VENC_H265_H264================\n");
    }
