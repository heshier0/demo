...
HI_VOID* SAMPLE_MISC_ViDump(VI_PIPE Pipe, VI_CHN Chn, HI_U32 u32FrameCnt, HI_U32 u32ByteAlign)
{
    ...
    while (HI_MPI_VI_GetChnFrame(Pipe, Chn, &stFrame, s32MilliSec) != HI_SUCCESS)
    {
        s32Times--;
    ...
    /* make file name */
    snprintf(szYuvName, 128, "./vi_pipe%d_chn%d_w%d_h%d_%s_%s_%s_%d_%d.yuv", Pipe, Chn,
    ...
    while (u32Cnt--)
    {
        ...
        if (bSendToVgs)
        {
            ...
            enBitWidth    = (DYNAMIC_RANGE_SDR8 == stFrame.stVFrame.enDynamicRange) ? DATA_BITWIDTH_8 : DATA_BITWIDTH_10;
